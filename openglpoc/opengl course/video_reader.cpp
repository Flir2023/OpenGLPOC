...
bool load_frame(const char* filename, int* width_out, int* height_out, unsigned char** data_out) {
    ...
    if (!av_format_ctx) {
        printf("No alloc format context");
        return false;
    }
    ...
    if (!av_codec_ctx) {
        printf("No alloc codec context");
        return false;
    }

    if (avcodec_parameters_to_context(av_codec_ctx, av_codec_params) < 0) {
        printf("No init codec context");
        return false;
    }

    if (avcodec_open2(av_codec_ctx, av_codec, NULL) < 0) {
        printf("No open codec");
        return false;
    }
...
bool video_reader_open(VideoReaderState* state, const char* filename) {
    ...
    std::cout << "Number of streams: " << av_format_ctx->nb_streams << std::endl;
    for (int i = 0; i < av_format_ctx->nb_streams; ++i) {
        av_codec_params = av_format_ctx->streams[i]->codecpar;
        av_codec = avcodec_find_decoder(av_codec_params->codec_id);
        if (!av_codec) {
...
bool video_reader_read_frame(VideoReaderState* state, uint8_t* frame_buffer, int64_t* pts) {
    ...
    while (av_read_frame(av_format_ctx, av_packet) >= 0) {
        ...
        std::cout << "read frame 1" << std::endl;
    ...
    *pts = av_frame->pts;
    ...
    try {
        std::cout << "Try swscale" << std::endl;
        sws_scale(sws_scaler_ctx, av_frame->data, av_frame->linesize, 0, av_frame->height, dest, dest_linesize);
    }
    catch (...) {
        std::cout << "I will arrive here" << std::endl;
    }
    return true;
}
